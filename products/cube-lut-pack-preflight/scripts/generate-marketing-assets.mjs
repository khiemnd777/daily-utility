...
import { cube1d, cube3d, zipFromMap } from "../tests/fixtures.mjs";
...
const fixturePath = fileURLToPath(new URL("fictional-lut-release.zip", workDirectory));
const appUrl = pathToFileURL(fileURLToPath(new URL("dist/index.html", productRoot))).href;
const releasePath = fileURLToPath(new URL("release/cube-lut-pack-preflight-v1.0.0.zip", productRoot));
const chromeExecutable =
  process.env.CHROME_EXECUTABLE || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const fixedTime = "2026-08-29T01:00:00.000Z";
...
const exact = cube1d(5, { title: "Clean Contrast" });
const fixture = zipFromMap({
  "clean/neutral-1d.cube": cube1d(4, { title: "Neutral 1D" }),
  "clean/warm-film.cube": cube3d(2, {
    title: "Warm Film",
    mapper: (red, green, blue) => [red, green * 0.95, blue * 0.9],
  }),
  "duplicates/clean-contrast.cube": exact,
  "duplicates/clean-contrast-copy.cube": exact,
  "equivalent/soft-a.cube": cube3d(2, { title: "Soft A", comments: ["# authoring note A"] }),
  "equivalent/soft-b.cube": cube3d(2, { title: "Soft B", comments: ["# authoring note B"] }),
  "README.txt": "Fictional buyer instructions for deterministic marketing evidence.",
});
...
function contentsSlide({ entries, size, sha256 }) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    *{box-sizing:border-box}body{margin:0;width:1600px;height:900px;overflow:hidden;background:radial-gradient(circle at 90% 5%,#ccf6f2 0,transparent 34rem),#f7f6fd;color:#171528;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    main{width:1480px;height:780px;margin:60px;padding:58px 64px;border:1px solid #d6d2e5;border-radius:30px;background:rgba(255,255,255,.96);box-shadow:0 24px 80px rgba(64,48,127,.14)}
    .eyebrow{margin:0 0 16px;color:#5135b5;font-weight:800;font-size:18px;letter-spacing:.12em;text-transform:uppercase}h1{margin:0;font:500 58px/1.02 Georgia,serif;letter-spacing:-.04em}.lede{max-width:1100px;margin:18px 0 34px;color:#646078;font-size:23px;line-height:1.5}
    ul{display:grid;grid-template-columns:1fr 1fr;gap:18px 26px;margin:0;padding:0;list-style:none}li{padding:24px 26px;border:1px solid #dfdcec;border-radius:15px;background:#faf9fe;font-size:22px;font-weight:750}small{display:block;margin-top:7px;color:#6d6880;font-size:15px;font-weight:500}
    footer{display:flex;justify-content:space-between;gap:30px;margin-top:30px;color:#666177;font-size:16px}.badge{color:#5135b5;font-weight:800}.hash{max-width:760px;text-align:right;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
...
try {
  ...
  await page.waitForFunction(
    () => window.__CUBE_LUT_PACK_PREFLIGHT__?.audit?.releaseName === "fictional-lut-release.zip",
    null,
    { timeout: 20_000 },
  );
  await page.evaluate(() => {
...
console.log("Generated deterministic buyer-facing evidence assets for CUBE LUT Pack Preflight.");
