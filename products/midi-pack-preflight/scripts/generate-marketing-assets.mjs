...
import { cleanFormat0, cleanFormat1, distinctPerformance, zipFromMap } from "../tests/fixtures.mjs";
...
const fixturePath = fileURLToPath(new URL("fictional-midi-release.zip", workDirectory));
const appUrl = pathToFileURL(fileURLToPath(new URL("dist/index.html", productRoot))).href;
const releasePath = fileURLToPath(new URL("release/midi-pack-preflight-v1.0.0.zip", productRoot));
const chromeExecutable = process.env.CHROME_EXECUTABLE
  || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const fixedTime = "2026-08-30T05:00:00.000Z";
...
const exact = cleanFormat0("Exact performance");
const fixture = zipFromMap({
  "chords/clean-chord.mid": cleanFormat0("Clean Chord"),
  "bass/clean-bass.midi": cleanFormat1(),
  "duplicates/exact.mid": exact,
  "duplicates/exact-copy.mid": exact,
  "duplicates/performance-a.mid": cleanFormat0("Performance A"),
  "duplicates/performance-b.mid": cleanFormat0("Performance B"),
  "distinct/lead.mid": distinctPerformance(),
...
function contentsSlide({ entries, size, sha256 }) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    *{box-sizing:border-box}body{margin:0;width:1600px;height:900px;overflow:hidden;background:radial-gradient(circle at 88% 5%,#d9f0e8 0,transparent 34rem),#f7f5ef;color:#171b1a;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .eyebrow{margin:0 0 16px;color:#176b53;font-weight:800;font-size:18px;letter-spacing:.12em;text-transform:uppercase}h1{margin:0;font:500 58px/1.02 Georgia,serif;letter-spacing:-.04em}.lede{max-width:1100px;margin:18px 0 34px;color:#59645f;font-size:23px;line-height:1.5}
    ...
    footer{display:flex;justify-content:space-between;gap:30px;margin-top:30px;color:#5f6964;font-size:16px}.badge{color:#176b53;font-weight:800}.hash{max-width:760px;text-align:right;overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
...
try {
  ...
  await page.waitForFunction(
    () => window.__MIDI_PACK_PREFLIGHT__?.audit?.releaseName === "fictional-midi-release.zip",
    null,
    { timeout: 20_000 },
  );
  await page.evaluate(() => {
...
console.log("Generated deterministic buyer-facing evidence assets for MIDI Pack Preflight.");
