...
### Legacy two-channel migration

New proposals and releases must not use the deprecated staged-release states. They remain readable only for products that entered the former two-channel workflow. If the owner explicitly removes the unresolved channel, Codex first unpublishes its seller listing and verifies that checkout is inactive while preserving the already verified Gumroad and KNA release. A focused product evidence change may then use schema-v3 `status: complete`, an empty `pending_channels` list, Gumroad-only sales evidence, and the required `retired_channels` plus `retirement` facts. `release-completed.yml` records `remaining_channels_cancelled` and moves the legacy issue to `PUBLISHED`; it does not call a seller or CMS API.
