...
When Lemon Squeezy becomes available for a staged release, Codex prepares or verifies the exact reusable `/checkout/buy/` URL, updates the reviewed Lemon Squeezy and KNA sources in a focused pull request, runs all required checks, and records `remaining_channels_ready` by moving the manifest and issue to `READY_FOR_REMAINING_CHANNELS`. `/approve` at that checkpoint authorizes only the exact remaining-channel materials: Lemon Squeezy publication of the already approved bytes and price, KNA page update, both Lemon Squeezy reciprocal links, buyer verification, and final release recording. `/request-changes` returns the issue to `GUMROAD_PUBLISHED` without unpublishing or mutating the Gumroad release.
