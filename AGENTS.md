...
New releases use Gumroad as the only sales and buyer-delivery channel, with KNA Software as the required first-party catalog. Do not create, review, publish, or advertise a second sales channel.

The deprecated `GUMROAD_PUBLISHED`, `READY_FOR_REMAINING_CHANNELS`, and `APPROVED_REMAINING_CHANNELS` states remain only to migrate products that entered the former two-channel workflow. When the owner explicitly retires the unresolved channel, Codex may complete `remaining_channels_cancelled` only after the seller listing is unpublished, its checkout is verified inactive, KNA exposes no link to it, and schema-v3 evidence records the retired channel and timestamp. This migration cannot add a destination, change approved Gumroad bytes or terms, or stand in for an unverified release.
...
- Flag a legacy channel-retirement completion unless the retired seller listing is inactive, KNA contains no retired-channel link, the verified Gumroad and KNA evidence remains unchanged, and the publication record contains the exact retirement evidence required by the schema.
