...
- Release approval authorizes final verification and merge of the linked product pull request, manual Gumroad and Lemon Squeezy publication of the exact approved version, publication of the exact reviewed KNA Software catalog page, all reciprocal-link checks, live buyer verification on both sales channels, creation and merge of the focused release-record pull request, and follow-through until the issue reaches `PUBLISHED`. It does not authorize external promotion posts.
...
- Before `READY_FOR_RELEASE`, add `products/<product-id>/marketing/knasoftware-listing.md` as the reviewed source of truth for every KNA Software CMS field, both sales destinations, and both reciprocal-link pairs, and list it in the manifest artifacts.
...
- Gumroad and Lemon Squeezy work is manual through the Codex desktop control plane. Publishing either channel requires `APPROVED_RELEASE` on the linked issue; the verified release `/approve` is the explicit instruction to publish the exact reviewed product version to both approved channels without another confirmation.
...
- KNA Software pages must use a stable lowercase product slug, self-canonical URL, descriptive purchase anchors for Gumroad and Lemon Squeezy, original page copy rather than a wholesale duplicate of either sales listing, and complete factual fields for product name, maker, audience, problem, version, price, features, limits, support, and both purchase destinations. Do not point the KNA canonical URL at a sales channel, misuse a documentation field as a purchase field, keyword-stuff, fabricate schema or reviews, or promise ranking gains.
...
- After publishing, verify both public sales pages, the shared USD price, buyer-visible delivery, support route, and downloaded artifact checksum on each channel. Then verify the KNA Software page links to both listings and both listings link back to KNA before creating v2 publication evidence or marking `PUBLISHED`.
...
- Flag a v2 release marked `PUBLISHED` without verified Gumroad and Lemon Squeezy listings, buyer-delivered checksums from both channels, a verified KNA Software product page, both sales-channel-to-KNA links, both KNA-to-sales links, or truthful SEO/AI-discovery evidence. Also flag external canonicals, misleading purchase links, duplicated keyword-stuffed copy, and claims of ranking improvement without measured evidence.
