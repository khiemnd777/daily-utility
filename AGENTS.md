...
`GUMROAD_PUBLISHED` means the exact approved bytes are live and verified on Gumroad, the KNA Software page is live with the Gumroad reciprocal-link pair, and Lemon Squeezy is still pending. It is not a completed release and does not authorize promotion. `remaining_channels_ready` supports two explicit review modes. `exact-url` requires the exact Lemon Squeezy listing, reusable checkout URL, shared price, artifact, KNA update, and reverse-link destination to be reviewable. `publish-bootstrap` is allowed only when the verified seller platform withholds the reusable URL until publication; it requires an exact unpublished Live draft and seller-side identifier, the expected storefront host and `/checkout/buy/` URL shape, shared price, artifact and checksum, listing copy, tax category, media, support terms, KNA reverse-link destination, and every KNA field and purchase anchor other than the platform-generated URL. The public KNA page must retain truthful pending copy and no placeholder link before approval. A fresh exact `/approve` from `READY_FOR_REMAINING_CHANNELS` authorizes only the selected remaining-channel review mode.
...
- Release approval authorizes the exact reviewed release sequence. For `simultaneous`, it authorizes final verification and merge of the linked product pull request, manual Gumroad and Lemon Squeezy publication, KNA Software catalog publication, every reciprocal-link and buyer check, and completion through `PUBLISHED`. For `gumroad-first`, it authorizes only the first stage: final product-PR verification and merge, Gumroad publication, the KNA page with its Gumroad link, live Gumroad buyer verification, and a partial release record ending at `GUMROAD_PUBLISHED`. The later Lemon Squeezy listing and KNA link update require a fresh `/approve` from `READY_FOR_REMAINING_CHANNELS`. In `publish-bootstrap` mode, that approval authorizes publishing only the exact reviewed Live draft, capturing the platform-generated reusable URL, accepting it only when its host and `/checkout/buy/` shape match the reviewed constraints, and then completing the reviewed KNA update and verification. Neither approval authorizes external promotion posts.
...
- Gumroad and Lemon Squeezy work is manual through the Codex desktop control plane. A simultaneous publication requires `APPROVED_RELEASE`. A reviewed Gumroad-first first stage also requires `APPROVED_RELEASE` and must stop at `GUMROAD_PUBLISHED`; Lemon Squeezy publication for that staged release requires `APPROVED_REMAINING_CHANNELS`. While the issue is `GUMROAD_PUBLISHED`, Codex may create or update an unpublished Lemon Squeezy Live draft using the already reviewed bytes, price, copy, support terms, and KNA destination. Use `exact-url` review when the draft exposes its reusable URL. If the platform demonstrably withholds that URL until publication, record `publish-bootstrap` with the exact Live draft identifier and all bounded URL and listing facts; do not publish merely to discover the URL before approval. Draft preparation does not authorize publication. Never infer either publish checkpoint from prose.
...
- In `publish-bootstrap` mode, publish the exact reviewed Live draft only after the issue reaches `APPROVED_REMAINING_CHANNELS`. Capture the dashboard Share URL without opening or converting it, require the reviewed storefront host and reusable `/checkout/buy/` shape, and persist the exact URL before final evidence. If the generated destination, checkout facts, backlink, price, artifact, or listing differs from the reviewed constraints, unpublish the Lemon Squeezy product, leave KNA unchanged, record `publish_bootstrap_failed`, return the issue and manifest to `GUMROAD_PUBLISHED`, and stop for a new review. The brief failed bootstrap must not be represented as a completed or verified release.
...
- Flag a staged release that exposes a placeholder or inactive Lemon Squeezy purchase link, records `GUMROAD_PUBLISHED` without verified Gumroad/KNA reciprocal evidence, advances the remaining channel without its second exact approval, or uses `publish-bootstrap` without an exact Live draft identifier and bounded storefront host/path review. Flag a bootstrap that continues after any generated URL or checkout mismatch instead of unpublishing and recording `publish_bootstrap_failed`. Flag any v2/v3 release marked `PUBLISHED` without verified Gumroad and Lemon Squeezy listings, buyer-delivered checksums from both channels, a verified KNA Software product page, both sales-channel-to-KNA links, both KNA-to-sales links, or truthful SEO/AI-discovery evidence. Also flag external canonicals, misleading purchase links, duplicated keyword-stuffed copy, and claims of ranking improvement without measured evidence.
