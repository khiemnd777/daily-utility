...
- A product already stranded in a deprecated remaining-channel state may use `remaining_channels_cancelled` only after the owner explicitly retires that channel, the seller listing is unpublished, checkout is verified inactive, KNA has no retired-channel link, and the focused publication record contains the required retirement evidence. Do not use deprecated staged states for a new product.
